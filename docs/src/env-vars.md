...
Controls whether the Azure Blob cache provider is enabled. When a compatible cache provider package is installed alongside `runai-model-streamer` and configured via `RUNAI_STREAMER_EXPERIMENTAL_AZURE_CACHE_PACKAGE`, the streamer auto-discovers and loads it at runtime to accelerate model loading from Azure Blob Storage via a distributed cache.
